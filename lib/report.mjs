...
// Storage kept is what a Repeat Visit is, and it is in the Report's own settings — the one place
// that rule is written. A Run read against a Repeat Visit would show the Worker's saving as if it
// were the page's, so the comparison names the pair rather than letting the deltas speak for
// themselves.
...
function knownArtifacts(report) {
  ...
  // A request whose transferSize is missing counts as having come down in full, not as reuse: a
  // missing figure is absence of evidence that anything was reused, and the mark's whole job is to
  // fire when a Repeat Visit cannot be shown to have returned. Unreachable on today's evidence —
  // every request item in every recorded Report carries a number — so nothing mutates it.
  const inFull = (request) => !(typeof request.transferSize === 'number') || request.transferSize > 0;
  if (isRepeatVisit(report) && requests.length > 0 && requests.every(inFull)) {
    artifacts.push({
      audit: 'network-requests',
      category: 'performance',
      reason:
        'every request came down in full, so nothing was reused: this Repeat Visit measured a first visit, which is what one looks like when the two navigations did not share a browser',
      scoreWithout: null,
    });
  }
  return artifacts;
