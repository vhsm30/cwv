...
// <picture> and <section> spans, the subresources the browser will fetch, the manifest the page
// links, and the inline stylesheet as rules that cascade per selector across every @media
// context. Nothing else in the repository should look at the page's bytes.
...
export async function loadPage(url) {
  return parsePage(await readFile(url, 'utf8'), url);
}

// The same model from a document already in hand: what a Preview URL served, read against the page
// on disk before a Run. Nothing here assumes the document is the Storefront's — a foreign one comes
// out with an empty title and no assets, which is how it is told apart.
export function parsePage(html, url = null) {
  ...
  // Assets are what the browser fetches to render the page. The manifest is not one of them: it is
  // fetched on its own terms (revalidated, not immutable), so it is a named fact rather than an asset.
  // A canonical is not even that — nothing fetches it, and it names where the page prefers to live
  // rather than anything served here — so it is a named fact and never a reference. The model
  // reports every one it finds; insisting there is exactly one is the contract's job, not the
  // parser's.
  const assets = [];
  const hrefs = [];
  const canonicals = [];
  ...
  for (const tag of tags) {
    ...
    if (name === 'link') {
      // Exactly canonical, nothing beside it: rel="canonical preload" IS fetched, so it stays a
      // reference like any other. rel is case-insensitive in HTML, so the comparison is too.
      if ((attrs.rel ?? '').trim().toLowerCase() === 'canonical') {
        ...
        if (/\bmanifest\b/.test(attrs.rel ?? '') && manifest === null) manifest = attrs.href ?? null;
      }
    }
  ...
  return {
    ...
    scripts,
    manifest,
    canonicals: canonicals.filter((href) => href !== undefined),
