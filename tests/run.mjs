...
// (refusing, naming, summarising, writing) is asserted here without ngrok or Chrome.
const REPORTS = new URL('../reports/', import.meta.url);
const PREVIEW_URL = 'https://pending-cozily-viscous.ngrok-free.dev/';
...
const newest = recorded.reduce((a, b) => (a.report.fetchTime > b.report.fetchTime ? a : b));
...
test('every recorded Report is a real Run of the Preview URL', () => {
  assert.ok(recorded.length >= 14, 'the 14 recorded Reports are the second adapter');
  for (const { name, report } of recorded) {
    assert.deepEqual(checkReport(report, PREVIEW_URL), [], name);
...
test('a Report that measured the ngrok interstitial is refused, with the reasons named', () => {
  const reasons = checkReport(interstitialReport(newest.report), PREVIEW_URL);
...
test('a Report of some other host is not a Run of this Preview URL', () => {
  const reasons = checkReport(newest.report, 'https://other.ngrok-free.dev/');
...
test('a Report of localhost is not a Run, because the throttling model assumes a real network hop', () => {
  const reasons = checkReport(rehost(newest.report, 'http://localhost:8000/'));
  assert.equal(reasons.length, 1);
...
test('a Report with a runtime error or a non-200 document is refused', () => {
  const errored = structuredClone(newest.report);
  ...
  const gone = structuredClone(newest.report);
...
test('the summary reads scores, metrics, requests, bytes, and the known artifacts of a Report', () => {
  const summary = summarize(newest.report);
...
test('the printed summary shows 0-100 scores, millisecond metrics, the artifact, and the Report name', () => {
  const text = formatSummary(summarize(newest.report));
  ...
  assert.ok(text.includes(reportName(newest.report)));
});

test('a Run writes its Report under the UTC name and returns the summary', async () => {
  const reportsDir = await scratchDir();
  const measure = recordedMeasure(new URL(newest.name, REPORTS));
  const { path: written, summary } = await performRun({ url: PREVIEW_URL, measure, reportsDir });
  assert.equal(path.basename(written), reportName(newest.report));
  assert.equal(path.dirname(written), reportsDir);
  assert.equal(JSON.parse(await readFile(written, 'utf8')).fetchTime, newest.report.fetchTime);
  assert.equal(summary.requests, 7);
  assert.deepEqual(await readdir(reportsDir), [reportName(newest.report)]);
});
...
test('a Run refuses an unreal Report and writes nothing', async () => {
  const reportsDir = await scratchDir();
  const measure = async () => interstitialReport(newest.report);
...
test('the command summarises a recorded Report without measuring anything', () => {
  ...
  const recordedFile = fileURLToPath(new URL(newest.name, REPORTS));
  const result = spawnSync(process.execPath, [command, recordedFile], { encoding: 'utf8' });
  assert.equal(result.status, 0, result.stderr);
  assert.equal(result.stdout.trim(), formatSummary(summarize(newest.report)));
});
